...
def get_date():  # 3分析处理 h八位列转日期格式
    ...
    if not df.empty:
        for C0L_DATE in df.columns:
            if max(df[C0L_DATE].astype("str").apply(len)) == 8:
                df.insert(0, "_转日期格式：" + C0L_DATE, df.apply(col2date, axis=1), allow_duplicates=True)
    dftofile(df, Fi1eOUT)
...
def idcard_add_18bit(df):  # apply函数：分析身份证 15转18 校验18、年龄、生日、性别
    ...
    return idcard, age, birthday, sex, area[0]
...
def col2date(df):  # apply函数：获取指定列日期格式
    date = str(df[C0L_DATE])
    if R4NGE:
        datesep = "/"
    else:
        datesep = "-"
    if len(date) == 8:
        date = date[0:4] + datesep + date[4:6] + datesep + date[6:8]
    return date
...
def idcard_analyse():  # 3分析处理 b分析身份证@ 指定单列
    ...
    if not df.empty:
        ...
        cols = ["_18位校验身份证", "_身份证年龄", "_身份证生日", "_身份证性别", "_身份证地区"]
...
def unzip(temp):  # 解压文件
    with ZipFile(file=Fi1eIN, mode='r') as zf:
...
def dirtozip(dirin, fileout):  # df写入压缩文件
    ...
    with ZipFile(zipout, 'w', ZIP_DEFLATED, allowZip64=True) as zf:
...
def file_read(filedir, filename, transform=True):  # 根据文件.扩展名读取文件，transform默认为真：要做预处理
    ...
    if os.path.getsize(filepath):  # 文件大小不等于0才执行
        if ".xls" in fileext:
            ...
            if filetext:
                with open(f"{Fi1eD1R}\\跳行警告{T1ME}：{filename}.txt", 'w') as openfile:
    ...
    for Fi1eD1R, dirs, files in os.walk(scandir):
        ...
        for Fi1eFULL in files:
            ...
            if not is_open():
                ...
                if range0:
                    R4NGE = re.sub(r"[{}]", "", range0[0])  # 提取大括号里的范围
                print("────────────────────────────────────────────────────────────────────────────────────")
